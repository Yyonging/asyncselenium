...
class title_is(object):
    ...
    async def __call__(self, driver):
        return self.title == await driver.title
...
class title_contains(object):
    ...
    async def __call__(self, driver):
        return self.title in (await driver.title)
...
class presence_of_element_located(object):
    ...
    async def __call__(self, driver):
        return await _find_element(driver, self.locator)
...
class url_contains(object):
    ...
    async def __call__(self, driver):
        return self.url in (await driver.current_url)
...
class url_matches(object):
    ...
    async def __call__(self, driver):
        import re
        match = re.search(self.pattern, await driver.current_url)
...
class url_to_be(object):
    ...
    async def __call__(self, driver):
        return self.url == await driver.current_url
...
class visibility_of_element_located(object):
    ...
    async def __call__(self, driver):
        try:
            return await _element_if_visible(await _find_element(driver, self.locator))
...
class visibility_of(object):
    ...
    async def __call__(self, ignored):
        return await _element_if_visible(self.element)
...
class presence_of_all_elements_located(object):
    ...
    async def __call__(self, driver):
        return await _find_elements(driver, self.locator)
...
class visibility_of_any_elements_located(object):
    ...
    async def __call__(self, driver):
        return [element for element in (await _find_elements(driver, self.locator)) if (await _element_if_visible(element))]
...
class visibility_of_all_elements_located(object):
    ...
    async def __call__(self, driver):
        try:
            elements = await _find_elements(driver, self.locator)
...
class text_to_be_present_in_element(object):
    ...
    async def __call__(self, driver):
        try:
            element_text = await (await _find_element(driver, self.locator)).text
...
class text_to_be_present_in_element_value(object):
    ...
    async def __call__(self, driver):
        try:
            element_text = await (await _find_element(driver,
    ...
    async def __call__(self, driver):
        try:
            if isinstance(self.frame_locator, tuple):
                await driver.switch_to.frame(await _find_element(driver,
                                                     self.frame_locator))
            else:
                await driver.switch_to.frame(self.frame_locator)
            return True
...
class invisibility_of_element_located(object):
    ...
    async def __call__(self, driver):
        try:
            target = self.target
            if not isinstance(target, AsyncWebElement):
                target = await _find_element(driver, target)
...
class element_to_be_clickable(object):
    ...
    async def __call__(self, driver):
        element = await visibility_of_element_located(self.locator)(driver)
...
class element_to_be_selected(object):
    ...
    async def __call__(self, ignored):
        return await self.element.is_selected()
...
class element_located_to_be_selected(object):
    ...
    async def __call__(self, driver):
        return await (await _find_element(driver, self.locator)).is_selected()
...
class element_selection_state_to_be(object):
    ...
    async def __call__(self, ignored):
        return (await self.element.is_selected()) == self.is_selected
...
class element_located_selection_state_to_be(object):
    ...
    async def __call__(self, driver):
        try:
            element = await _find_element(driver, self.locator)
            return (await element.is_selected()) == self.is_selected
        except StaleElementReferenceException:
            return False
...
class number_of_windows_to_be(object):
    ...
    async def __call__(self, driver):
        return len(await driver.window_handles) == self.num_windows
...
class new_window_is_opened(object):
    ...
    async def __call__(self, driver):
        return len(await driver.window_handles) > len(self.current_handles)
...
async def _find_element(driver, by):
    """Looks up an element. Logs and re-raises ``WebDriverException``
    if thrown."""
    try:
        return await driver.find_element(*by)
...
async def _find_elements(driver, by):
    try:
        return await driver.find_elements(*by)
    except WebDriverException as e:
        raise e
